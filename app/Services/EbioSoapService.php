...
class EbioSoapService
{
    ...
    public function lockDoor(Organisation $organisation, string $serialNumber): bool
    {
        if (empty($organisation->ebio_url) || empty($organisation->ebio_soap_username)) {
            throw new \Exception('eBioServer SOAP credentials are not configured for this organisation.');
        }
        ...
        <soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
          <soap:Body>
            <DeviceCommand_LockDoor xmlns="http://tempuri.org/">
              <UserName>'.$organisation->ebio_soap_username.'</UserName>
              <Password>'.$organisation->ebio_soap_password.'</Password>
              <DeviceSerialNumber>'.$serialNumber.'</DeviceSerialNumber>
            </DeviceCommand_LockDoor>
          </soap:Body>
        </soap:Envelope>';

        $response = Http::withHeaders([
            'Content-Type' => 'text/xml; charset=utf-8',
            'SOAPAction' => '"http://tempuri.org/DeviceCommand_LockDoor"',
        ])->send('POST', $url, [
            'body' => $xml,
        ]);
        ...
        if (str_contains($body, '<DeviceCommand_LockDoorResult />')) {
            return true;
        }

        preg_match('/<DeviceCommand_LockDoorResult>(.*?)<\/DeviceCommand_LockDoorResult>/', $body, $matches);
        $result = $matches[1] ?? '';

        return strtolower($result) === 'success' || empty($result) && $result !== 'error';
    }

    public function normalDoor(Organisation $organisation, string $serialNumber): bool
    {
        if (empty($organisation->ebio_url) || empty($organisation->ebio_soap_username)) {
            throw new \Exception('eBioServer SOAP credentials are not configured for this organisation.');
        }
        ...
        <soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
          <soap:Body>
            <DeviceCommand_NormalDoor xmlns="http://tempuri.org/">
              <UserName>'.$organisation->ebio_soap_username.'</UserName>
              <Password>'.$organisation->ebio_soap_password.'</Password>
              <DeviceSerialNumber>'.$serialNumber.'</DeviceSerialNumber>
            </DeviceCommand_NormalDoor>
          </soap:Body>
        </soap:Envelope>';

        $response = Http::withHeaders([
            'Content-Type' => 'text/xml; charset=utf-8',
            'SOAPAction' => '"http://tempuri.org/DeviceCommand_NormalDoor"',
        ])->send('POST', $url, [
            'body' => $xml,
        ]);
        ...
        if (str_contains($body, '<DeviceCommand_NormalDoorResult />')) {
            return true;
        }

        preg_match('/<DeviceCommand_NormalDoorResult>(.*?)<\/DeviceCommand_NormalDoorResult>/', $body, $matches);
        $result = $matches[1] ?? '';
